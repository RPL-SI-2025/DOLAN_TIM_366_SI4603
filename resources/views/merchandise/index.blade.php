...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 mt-5">
  ...
  @else
    @foreach ($merchandises as $item)
      <div
        class="w-full max-w-sm drop-shadow-lg bg-white border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700 overflow-hidden transition duration-300 ease-in-out hover:shadow-xl mx-auto flex flex-col">
        <a href="{{ route('merchandise.show', $item->id) }}" class="block flex-shrink-0">
        ...
        <div class="p-5 flex flex-col flex-grow justify-between">
          <div>
            <a href="{{ route('merchandise.show', $item->id) }}">
              ...
              <h5
                class="mb-2 text-xl font-bold tracking-tight text-purple-900 dark:text-white flex items-center gap-2">
            ...
            <div class="flex items-center mb-2">
              <p class="text-xs font-normal text-gray-700 dark:text-gray-400">Rp
                {{ number_format($item->price, 0, ',', '.') }}</p>
            </div>
          </div>
          <a href="{{ route('merchandise.show', $item->id) }}"
            class="inline-block text-white bg-gradient-to-br from-purple-400 to-black hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-gray-200 dark:focus:ring-purple-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center mt-4">
            Lihat Detail
          </a>
        </div>
      </div>
